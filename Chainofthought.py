...
class Chainofthought():
    def __init__(self, LLM = GPT(), mode="normal"):
    ...
    def _generate_cot(self,result:str):  
        ...
        for p in paragraphs[1:]:
            ...
            if first_line.startswith("analysis_step") or first_line.startswith("Analysis_step"):
                filtered_paragraph.append(p)

        return '\n\n'.join(filtered_paragraph)
    
    # COT construction process
    def cot_analysis(self, VulClass, code)->list:
        result = []
        for vul in VulClass:
            try:
                chain = self.Knowledge_set[vul]
            except KeyError:
                print(vul)
                print("unknown use")
                chain = self.Knowledge_set["auto_prompts"]  
            if self.mode == "strict" or self.mode == "abstrict":
                if vul == "auto_prompts":
                    marks = "potential"
                else:
                    marks = vul
                ...
                '''
            if self.mode == "ease" or self.mode == "abease":
                Template = f'''
                    You are now a security expert familiar with Linux OS and CVE/CWE security knowledge.
                ...
                Template = f'''
                    You are now a security expert familiar with Linux OS and CVE/CWE security knowledge.
                    Please determine whether the following function fragment limited by triple backticks is vulnerble. 
                    The determination should follow a strict review method.
                    ```{code}```
                    strictly follow the steps below "{chain}" think through the question. 
                    You must add "@@" at the end of your answer paragraph
                '''
            ...
            while True:
                if iter > 1:
                    print(answer)
                    print("Try to generate the correct chain...")
                ...
                if "@@" in answer.lower():
                    if answer.endswith("@@"):
                        break
                if iter > 5:
                    raise ValueError("The generation result of cot does not meet the standards")
            if vul == "unknown":
                cot = self._generate_normal_cot(answer)
                result.append(vul+"-->"+"\n"+cot)
                continue
            if vul == "auto_prompts":
                cot = self._generate_normal_cot(answer)
